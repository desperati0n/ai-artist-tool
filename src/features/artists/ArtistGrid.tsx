...
export function ArtistGrid() {
  ...
  // Legacy archives can contain repeated IDs. Include the list position so React
  // can remove every old card when paging, even when those IDs collide.
  const itemKey=(id: string | number,index: number) => JSON.stringify([s.viewMode,(page-1)*s.pageSize+index,id]);
  ...
  return <>
    <div ref={container} id="grid-container" className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-slate-50/50 dark:bg-slate-900/20">
      {items.length===0 ? <div className="h-full flex flex-col items-center justify-center text-slate-400"><Icon name={s.viewMode==='presets'?'package-open':'ghost'} className="empty-state-icon opacity-30" /><p>{s.viewMode==='presets'?'暂无预设，请在右侧保存':'没有数据'}</p></div> : <div className={`grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 ${s.viewMode==='artists'?'xl:grid-cols-6':''} gap-4 pb-4`}>{items.map((item,index) => s.viewMode==='artists'?<ArtistCard key={itemKey(item.id,index)} artist={item as Artist} />:<PresetCard key={itemKey(item.id,index)} preset={item as Preset} />)}</div>}
    </div>
