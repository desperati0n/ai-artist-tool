/*
 * NAI Batch Updater plugin for AI Artist Manager.
 * It intentionally has no build step: copy this file into the manager's
 * `plugins/` directory and keep the script tag in index.html.
 */
(function () {
  'use strict';
  ...
  const manager = () => {
    const managerState = typeof state !== 'undefined' ? state : null;
    const managerActions = typeof actions !== 'undefined' ? actions : null;
    const managerDb = typeof db !== 'undefined' ? db : null;
    return { state: managerState, actions: managerActions, db: managerDb };
  };
  ...
  function loadJsZip() {
    ...
    return new Promise((resolve, reject) => {
      ...
      script.onerror = () => reject(new Error('无法加载图片解包组件 JSZip，请检查网络连接。'));
  ...
  async function generateOne(artist) {
    ...
    const zip = await window.JSZip.loadAsync(responseBlob);
  ...
  async function settleReview(result, approved) {
    ...
    if (approved) {
      await managerContext.db?.put(result.artist.id, result.dataUrl);
      if (managerContext.state?.pageImages) managerContext.state.pageImages[id] = result.dataUrl;
  ...
  window.NAIBatchPlugin = { open, close };
})();
